#include <iostream>
#include <stdio.h>
#include <string.h>
#define MAXLEN 3000 
using namespace std;
int a[MAXLEN][MAXLEN];
int b[MAXLEN][MAXLEN];
int c[MAXLEN][MAXLEN];
int d[MAXLEN][MAXLEN];
//para c[i][j] stands for Xi&Yj Maximum common sequence
//para b[i][j] used to traceback to construct the onScreen info
void LCSLength(char *x,char *y,int m,int n,int c[][MAXLEN],int b[][MAXLEN])
{
	int i,j;
	for(i=0;i<=m;i++)
		c[i][0]=0;
	for(j=1;j<=n;j++)
		c[0][j]=0;
	for(i=1;i<=m;i++)
	{
		for(j=1;j<=n;j++)
		{
			if(x[i-1]==y[j-1])
			{
				c[i][j]=c[i-1][j-1]+1;
				b[i][j]=0;
			}
			else if(c[i-1][j]>=c[i][j-1])
			{
				c[i][j]=c[i-1][j];
				b[i][j]=1;
			}
			else
			{
				c[i][j]=c[i][j-1];
				b[i][j]=-1;
			}
		}
	}
}

void PrintSeq(int b[][MAXLEN],char *x,int i,int j)
{
	if(i==0||j==0)
		return;
	if(b[i][j]==0)
	{
		PrintSeq(b,x,i-1,j-1);
		printf("%c",x[i-1]);
	}
	else if(b[i][j]==1)
		PrintSeq(b,x,i-1,j);
	else
		PrintSeq(b,x,i,j-1);
}

//int main(void)
//{
//	char A[MAXLEN]=
//	"";
//	char B[MAXLEN]=
//	"&(!#a%$(n@*%+!*&a)*$l%&&g))*o)($r&@$i%#!t)$$h^#$m@!!+@&!i$%!s!#(+!)&a)(%n(()y*)@+%()w^%&e$!!l%@&l*@(d@*)e!(*f#)@i%)^n*%*e^@(d^*%+&^)c@*#o()(m!(^p##)u&%#t#(^a*&%t@%$i%!@o@*%n^@&a$%)l@($+*%^p##^r@&%o%!$c#^!e%!(d!)!u%$^r%^%e$@)+@$^t^&&h&!*a!!%t$^*+&^!t#$@a(^!k)(#e$(#s#!(+!#*s%!)o!^%m#!(e$*^+#($v*!*a)#%l#$#u#)#e&%&s%&$+#^(a@$(s&**+(!@i^$)n$@)p!!*u%%$t&#@+%*!a#%%n!^#d@$$+!*)p@$%r$$@o*!%d#&(u@&&c!#*e^^%s&$%+@!%s&!^o@(&m&(&e$!*+$!&v%)!a#%^l#(&u&%$e$@&s)@*+^(*a^!^s)!!+#%#o^%%u&($t%&%p#()u&^#t&^!^@@($*@$(&)&(*!#%^@!^^$*&!^*!!))##$*$)#$!^$@$!^&$#(#%(*^@!@!*)^!&$*@$%$%%!(&&^@)&*))&^&&@&)#(^*$&#^$#%^%#^&@&*#&!(#*%#**$@#)^(*()@(^&!@&(#^#@%$&)^^($!(($(!#^)&!%#@!&%&@#%!!%(^^&!!#(%*$##&()@!(!*#%*^^!*!*^&*&()#%^*&(&!^@$(@^)@@$$%!$$*$$@)*#@($(%(*$$##)$!@!)&@@!$!&@&&)()(^&&&&%((!@$%^!@^(!&$@$@^$$#$$!^@))##($*@$&@^$^$@*^^*!%@$&!$)^$)*#%^!@(@$%*#&&&(^(**)$^)(!)&$^!*$&$@%(^**$%&@%#^@@^#(!(%^))*@@###*@^@&*$(#&)%(%!^&!*)^%!@^)%&!$@$$#!)&!!&*!!%)@@&@(*@(^!$&$#!)))!!^^%@%(**)$*^!!*($!*!)!#^@%*&$$%!%)#$!($()%(%)!#";
//	char C[MAXLEN]=
//	"";
//	char D[MAXLEN]=
//	"#%#&a!%&n*%*+##%a)$!l*!(g)&*o%&!r&%(i^!(t))%h!(!m!#!+$*$i)!@s*!@+)^(a%)$n)%@y)^@+*^#w&!^e&&#l#$&l)&$d!&!e@@%f@&&i)#&n^)#e#*&d*!)+^&$c$!)o@&!m^!#p@#!u*$#t@!(a*^@t##!i#$$o)^#n&@)a()*l@$!+$))p*!*r)$(o#^(c%$#e#@^d!)&u*$#r%)(e&*)+%*&t@$^h*^$a**&t)#!+(@)t^!)a%@&k!!@e^*&s^#&+%)%s$*(o)@!m%)^e((*+^^$v#^^a*#*l$&*u)#@e(($s@)*+!!)a)&(s%&@+&!)i%@&n%*)p*%^u(#@t(!!+(&&a^^#n@()d%$%+)$%p&()r)!!o^&%d%&^u!!!c&&$e*)^s!%!+^!*s!)!o%(*m%((e^@@+@($v!!)a())l*!*u)(!e&*$s)(*+$^!a!%^s$@(+^!^o)#)u!%$t^^(p)$^u(&$t@^)##@(&))(%)#@*&$$^###&##&#(#%$$!&$#$&*^^!$(%$#@((@!)!^%$*(&)@*&)#^^@%$*(^&%@^^())^!^$#($%@!@)*@!^#$@@)()#^^)*)@^&((**@%**)(#*@^)!)(^%(!!(()#@(&^%&#))*@#(^&(@#$@!%%&#@)^*&^($()^*@!$)*&@%^@$&^!$(**(&!@&&!#*&))(^$&()#%@$!%!)#)#^!^@@%!)#*)#$*%^@&!*&!%!((!^*&&&@*$&#!^^*%((($%!!&^(&)&(#^$%#&*&$)&^!@#)*#)($(#$%^!^)$)*&*!$$*^@^^^))^*$)$!^^%!(!^)@#*))%^(^^)#*$#%^!!!*!!$)!^^)@#&&$(&*(*$(^$(*))(&^&(%)**)))!((!!&**@@#$#(*)$#$^#%!^*^*%&!)$(!!(&)^(^#^%(^^*&$)**)(@!#$(@%@$!%*%*%@@%%*)*!)^!#!@!^@##%(@!*$$^(!***#!#(*!^)(%&^&(&@#%#&^^*$";
//	int m,n,i,j;
//	m=strlen(A);
//	n=strlen(B);
//	i=strlen(C);
//	j=strlen(D);
//	LCSLength(A,B, m, n, b, a);
//	PrintSeq(a, A, m, n); 
//	printf("\n=======================================================================================================\n");
//	LCSLength(C, D, i, j, d, c); 
//	PrintSeq(c, C, i, j);
//	printf("\n=======================================================================================================\n");
//	LCSLength(A,D, m, j, d, a); 
//	PrintSeq(a, A, m, j); 
//	printf("\n=======================================================================================================\n");
//	LCSLength(C, B, i, n, b, c); 
//	PrintSeq(c, C, i, n);
//	return 0;
//}

